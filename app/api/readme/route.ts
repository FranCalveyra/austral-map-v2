...
export async function GET() {
  try {
    ...
    let htmlContent = readmeContent
      // Headers
      .replace(/^### (.*$)/gm, '<h3 class="text-lg font-semibold text-gray-800 mb-2">$1</h3>')
      .replace(/^## (.*$)/gm, '<h2 class="text-xl font-semibold text-gray-800 mb-3">$1</h2>')
      ...
      .map(paragraph => {
        ...
        if (paragraph.includes('<li')) {
          return `<ul class="space-y-1 text-gray-600">${paragraph}</ul>`;
        }
        if (paragraph.includes('<img')) {
          return `<div class="flex flex-wrap items-center gap-1 mb-2">${paragraph}</div>`;
        }
        return `<p class="text-gray-600 leading-relaxed mb-4">${paragraph.replace(/\n/g, '<br/>')}</p>`;
      })
      .join('\n');
